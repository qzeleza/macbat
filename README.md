...
<img src="images/sys-tray-image.png" alt="macbat in system tray" width="390" />
...
```bash
# Сначала подключите репозиторий (tap)
...
Для обновления приложения используйте следующие команды:
...
- **Пороги отслеживания**: Показывает установленные в конфигурации минимальный и максимальный пороги заряда, которые можно изменить вручную.
- **Пороговые интервалы**: Показывает временные интервалы проверки состояния батареи в зависимости от статуса зарядки.
- **Настройки и журнал**: Открывает файл `config.json` для редактирования и файл `macbat.log` для просмотра логов приложения.
...
```json
{
  "min_threshold": 21,   
  "max_threshold": 81,
  "check_interval_charging": 15,
  "check_interval_discharging": 110,
  "notification_interval": 180,
  "max_notifications": 3,
  "debug_enabled": false,
  "log_file_path": "/Users/samovar/Library/Logs/macbat/macbat.log",
  "log_rotation_lines": 200,
  "log_enabled": true,
  "run_at_load": true
}
```
...
- `log_rotation_lines`: Количество строк в файле логов перед вращением.
- `log_enabled`: Включает или отключает логирование.
- `run_at_load`: Запускает приложение при загрузке системы.
...
Вы также можете помочь проекту переводом средств на [этот кошелек ЮМани](https://yoomoney.ru/to/4100117756734493).
